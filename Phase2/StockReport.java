...
public class StockReport extends InventoryDriver {
	...
	public class StockReportHandler implements EventHandler<ActionEvent> {  // handles each event in StockReport scene according to specifications
		...
		private void handleExportBtn() {
			if(fileNameTF.getText().length() == 0) return;
			try {
