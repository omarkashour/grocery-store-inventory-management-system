...
public class Inventory {
	...
	public String formatedStockReport() { // generates a properly formatted stock report for storing on a file or
											// database and re-reading later
		double value = 0;
		String report = "";
		for (int i = 0; i < inventory.size(); i++) {
			if (inventory.get(i) instanceof Brand) { // check from subclasses first in ascending order to super class to
														// avoid wrong output (Polymorphism)
				Brand brand = (Brand) inventory.get(i);
				report += brand.getBrand() + " " + brand.getType() + " " + brand.getQuantity() + " "
						+ String.format("%.2f", brand.getPrice()) + "\n";
			} else if (inventory.get(i) instanceof Item) {
				Item item = inventory.get(i);
				report += item.getType() + " " + item.getQuantity() + " " + String.format("%.2f", item.getPrice())
						+ "\n";
			}
		...
		// format type for brand: brand item quantity price
		// format type for item: item quantity price
	}
